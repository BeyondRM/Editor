package brm.editor.database;
import brm.editor.database.items.TemplateSubtypes;
import brm.editor.database.items.TemplateVariable;
import java.net.URL;
import java.util.List;
import java.util.ResourceBundle;
import javafx.collections.ObservableList;
import javafx.fxml.FXML;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.control.TableColumn;
import javafx.scene.control.TableView;
import javafx.scene.control.TextField;
import javafx.scene.layout.AnchorPane;
import javafx.scene.layout.HBox;
import javafx.scene.layout.VBox;


/**
 * FXML Controller class
 *
 * @author Gregory
 */
public final class TemplateVariableListController {
  // special auto-injected values:
  @FXML
  private URL location;
  @FXML
  private ResourceBundle resources;
  // Other FXML components/fields
  @FXML
  private AnchorPane anchorPane;
  @FXML
  private Button button;
  @FXML
  private HBox hBox;
  @FXML
  private Label label;
  @FXML
  private TableColumn<TemplateVariable, Integer> column1;
  @FXML
  private TableColumn<TemplateVariable, String> column3;
  @FXML
  private TableView<TemplateSubtypes> tableView;
  @FXML
  private TextField textFieldAbbr;
  @FXML
  private TextField textFieldName;
  @FXML
  private VBox vBox;
  // non-fxml:
  private List<String> strings;
  private String baseAbbr;
  private String baseName;
  private boolean changed;

  public TemplateVariableListController() {
  }

  @FXML
  private void initialize() {
  }

  @FXML
  public void modifyTextFieldAbbr() {
  }

  @FXML
  public void modifyTextFieldName() {
  }

  @FXML
  public void saveChange() {
    ObservableList<TemplateSubtypes> items = tableView.getItems();
    int index = tableView.getSelectionModel().getSelectedIndex();
    if(!items.get(index).abbr.equals(textFieldAbbr.getText())) {
      items.get(index).abbr.set(textFieldAbbr.getText());
    }
    if(!items.get(index).name.equals(textFieldName.getText())) {
      items.get(index).name.set(textFieldName.getText());
    }
  }

  public void setLabel(String s) {
    label.setText(s);
  }
}
