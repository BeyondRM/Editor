...
public class MyApplicationClass extends Application {
  ...
  public void start(final Stage stage) throws Exception {
    ...
    // continuously refresh the TreeItems. demonstrates using controller methods to manipulate the controlled UI.
    final Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(3), new TreeLoadingEventHandler(controller)));
    timeline.setCycleCount(Timeline.INDEFINITE);
    timeline.play();

    // close the app if the user clicks on anywhere on the window. just provides a simple way to kill the demo app.
  ...
  private class TreeLoadingEventHandler implements EventHandler<ActionEvent> {
    private final MyControllerClass controller;
    private int idx = 0;
